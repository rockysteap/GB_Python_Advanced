...
# Если файл существует, открываем его для записи и удаляем данные, которые в нём хранились.
...
# Если каждая строка должна сохранятся в файле с новой строки,
# необходимо самостоятельно добавить символ переноса - \n
"""
text = ['Lorem ipsum dolor sit amet, consectetur adipisicing elit.',
...
# воспользовались строковым методом join, так как writelines не добавляет
# переноса между элементами последовательности.


# ● print в файл

# Функция print по умолчанию выводит информацию в поток вывода.
# Обычно это консоль. Но можно явно передать файловый объект для печати в файл.
# Для этого надо воспользоваться ключевым параметром file.
"""
text = ['Lorem ipsum dolor sit amet, consectetur adipisicing elit.',
        'Consequatur debitis explicabo laboriosam sint suscipittemporibus veniam?',
        'Accusantium alias amet fugit iste neque non odit quiasaepe totam velit?', ]
