...
# Дочитав файл в цикле while до конца изменяем позицию “курсора” на начало последней строки и начинаем запись.
# Таким образом мы сохранили все строки файла кроме последней и записали новый текст в конец.
...
# Метод возвращает позицию после изменения файла.
"""
last = before = 0
with open('new_data.txt', 'r+', encoding='utf-8') as f:
    while line := f.readline():
...
# Если size больше размера файла, он увеличивается до указанного размера.
# При этом добавленный размер обычно заполняется нулевыми байтами.
# Заполнение зависит от конкретной ОС.
