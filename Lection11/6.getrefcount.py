# Команда del
# Зарезервированное слово del не удаляет объекты в Python.
# Оно разрывает связь между переменной и объектом, на который переменная указывает.
# После строки с del к переменной нельзя обратиться,
# а у объекта на единицу уменьшается счётчик ссылок.
